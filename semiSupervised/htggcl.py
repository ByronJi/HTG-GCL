...
from semiSupervised.model import SimCLRWithTMDC
...
def get_all_embeddings(model, dataloader, device):
    ...
    for data_batch, complex_batch_list in dataloader:
        ...
        with torch.no_grad():
            features = model.forward_common_projected(data_batch, complex_batch_list)
            z_list = [features[f'view{i}'] for i in range(model.num_views)]
            z = torch.cat(z_list, dim=1)  # 视图拼接 concat
        all_z.append(z.cpu())
        all_y.append(data_batch.y.cpu())
