...
from semiSupervised.evaluate_embedding import semi_evaluate_embedding
...
if __name__ == "__main__":
    ...
    train_loss_curve = []
    params = []
    # accuracies = {'val': [], 'valc1':[], 'valc2':[], 'valc3':[], 'test': [], 'testc1':[], 'testc2':[], 'testc3':[]}
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % 5 == 0:
            ...
            # acc_val_c1, acc_c1 = evaluate_embedding(emb_c1, y)
            # acc_val_c2, acc_c2 = evaluate_embedding(emb_c2, y)
            # acc_val_c3, acc_c3 = evaluate_embedding(emb_c3, y)
            # acc_cat_val, acc_cat = evaluate_embedding(emb_cat, y)
            # acc_mean_val, acc_mean  = evaluate_embedding(emb_mean, y)
            acc_cat_val, acc_cat = semi_evaluate_embedding(emb_cat, y)
            acc_mean_val, acc_mean = semi_evaluate_embedding(emb_mean, y)

            # accuracies['val'].append(acc_val)
            # accuracies['test'].append(acc)
            # accuracies['valc1'].append(acc_val_c1)
            # accuracies['testc1'].append(acc_c1)
            # accuracies['valc2'].append(acc_val_c2)
            # accuracies['testc2'].append(acc_c2)
            # accuracies['valc3'].append(acc_val_c3)
            # accuracies['testc3'].append(acc_c3)
    ...
    #     f"Test max:\n{max(accuracies['test'])}\n"
    #     f"accuracies[valc1]: {accuracies['valc1']}\n"
    #     f"accuracies[testc1]: {accuracies['testc1']}\n"
    #     f"Test1 max:\n{max(accuracies['testc1'])}\n"
    #     f"accuracies[valc2]: {accuracies['valc2']}\n"
    #     f"accuracies[testc2]: {accuracies['testc2']}\n"
    #     f"Test2 max:\n{max(accuracies['testc2'])}\n"
    #     f"accuracies[valc3]: {accuracies['valc3']}\n"
    #     f"accuracies[testc3]: {accuracies['testc3']}\n"
    #     f"Test3 max:\n{max(accuracies['testc3'])}\n"
    # )
    msg = (
        f'========Params========\n'
        f'args: {args}\n'
